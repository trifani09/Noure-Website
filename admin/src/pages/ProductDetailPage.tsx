...
export function ProductDetailPage({ publicId }: { publicId: string }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-3"><PageHeader title={product.name} description={product.short_description ?? 'Product details'} /><div className="flex gap-2"><a href={`/products/${publicId}/variants`} className="rounded-lg border px-4 py-2 text-sm">Manage variants</a><a href={`/products/${publicId}/edit`} className="rounded-lg bg-stone-900 px-4 py-2 text-sm text-white">Edit product</a></div></div>
