...
class ShippingApiTest extends TestCase
{
    ...
    {
        [$token] = $this->cart(1);
        $this->withUnencryptedCookie('noure_cart', $token)->getJson('/api/v1/checkout/shipping-methods?country_code=ID&city=Jakarta&province=DKI%20Jakarta')
    ...
    {
        [$token] = $this->cart(4);
        $this->withUnencryptedCookie('noure_cart', $token)->getJson('/api/v1/checkout/shipping-methods?country_code=ID&city=Bandung&province=Jawa%20Barat')
    ...
    {
        [$token] = $this->cart(1);
        $this->withUnencryptedCookie('noure_cart', $token)->getJson('/api/v1/checkout/shipping-methods?country_code=US&city=New%20York')
        ...
        $this->withUnencryptedCookie('noure_cart', $token)->postJson('/api/v1/orders', $this->orderPayload('express'))
    ...
    {
        [$token] = $this->cart(1);
        $response = $this->withUnencryptedCookie('noure_cart', $token)->postJson('/api/v1/orders', $this->orderPayload('express') + ['shipping_amount' => 1, 'grand_total_amount' => 1])
