...
export function ProductGallery({ product }: { product: ProductDetail }) {
  ...
  if (!images.length)
    return (
      <div className="grid aspect-[4/5] place-items-center bg-ivory text-xs uppercase tracking-widest text-taupe">
        Noure
      </div>
    );
        ...
        >
          {images.map((image, index) => (
            <button
              ...
              className={`focus-ring relative aspect-[4/5] w-20 shrink-0 snap-start overflow-hidden border-2 ${index === active ? "border-ink" : "border-transparent opacity-70 hover:opacity-100"}`}
            >
              <SafeImage
                fill
                sizes="80px"
